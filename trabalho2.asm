.data
	...
	MENU_ESCOLHA: .asciiz 	" Escolha uma das opções abaixo: "
	OPCAO_ADICIONAR_FIGURINHA: .asciiz 	"1- Adicionar figurinha à coleção."
	OPCAO_RESETAR: .asciiz 	"2- Recomeçe seu álbum do zero." 
	OPCAO_SAIR: .asciiz 	"3- Sair do programa."
	CONFIRMA_RESET: .asciiz 	" Você tem certeza que quer excluir seu progresso e começar de novo? (Ação PERMANENTE)"
	INSTRUI_USUARIO_RESET1: .asciiz 	"1- Não."
	INSTRUI_USUARIO_RESET2: .asciiz 	"2- Sim."
	RESET_CONCLUIDO: .asciiz 	"Reset concluído com sucesso!"
	INVALIDA: .asciiz 	"Entrada Inválida."
...
adicionar_figurinha:
	...
	upper_case:
		...
		loop:
			add	$t0,$t2,$t1
			lb	$t3, 0($t0)        #Carrega caractere a caractere as letras do input do usuário
...
remove:
	#Vai carregar cada caracter do input em registradores diferentes. ($t5 até $t9 respectivamente)
	...
	lendo_caracter:
		...
		#Aqui tem que ter a logica para excluir do arquivo ($t0 contem o inicio da carta)
				...
				#usar para trocar por espaços em branco
		li	$t1,0
		add	$t4,$t0,$t1
		sb	$s6,0($t4)
		addi	$t1,$t1,1
	...
	caracter_diferente:
		li	$t1,0
		addi	$t0,$t0,6 #Vai para onde começa a proxima carta 
		
		slt 	$t2,$t0,$s7   #3835 eh onde começa a ultima carta
